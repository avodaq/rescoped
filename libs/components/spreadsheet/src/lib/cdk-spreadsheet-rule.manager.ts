...
import { GlobalRules, ItemActionType, ItemRules, RuleTypes } from './cdk-spreadsheet-data.manager';
import { getItemPayload } from './cdk-spreadsheet.utils';
import { AbstractControl } from '@angular/forms';

@Injectable()
export class CdkSpreadsheetRuleManager<Item> {
  ...
  getActionRule<ItemKey extends keyof Item, ActionType extends ItemActionType>(
    ...
    if (typeof action?.componentType === 'undefined') {
      throw new Error(
        `At least component must be defined in action rule for key=${String(
          key,
        )}, actionType${actionType}`,
      );
    }
  ...
  applyRules<ItemKey extends keyof Item, ActionType extends ItemActionType>(
    ...
    this.#runRules(rule, formControl, initialValue);
  }
